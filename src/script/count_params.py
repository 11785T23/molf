...
def main():
    ...
    parser.add_argument("--use_fft", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--freeze_non_linear", action=argparse.BooleanOptionalAction, default=False)
    args = parser.parse_args()

    print(f"\n{'='*70}")
    print(f"Model: {args.model_name_or_path}")
    print(f"MoLF lora_ranks: {args.molf_lora_ranks}")
    print(f"MoLF fft: {args.use_fft}")
    print(f"Freeze non-linear: {args.freeze_non_linear}")
    print(f"{'='*70}")
    ...
    molf_cfg = MoLFConfig(
        ...
        molf_fft=args.use_fft,
        freeze_non_linear=args.freeze_non_linear,
    )
