...
class MoLFConfig(BaseModuleConfig):
    ...
    # MoLF-Efficient: when False, the base weight is frozen (no FFT expert) and
    # routing happens purely over the LoRA experts in `lora_experts_ranks`.
    molf_fft: bool = True
    # Also freeze non-linear (norm/embedding/bias) parameters left outside the
    # wrapped linears, so only the LoRA experts train.
    freeze_non_linear: bool = False
