...
class TrainingConfig:
    ...
    molf_topk: int = 1
    # MoLF-Efficient (§4.4): set molf_fft=False to freeze the base weight (drop
    # the FFT expert) and route purely over the LoRA experts in molf_lora_ranks.
    # molf_freeze_non_linear additionally freezes the leftover norm/embedding
    # parameters so that only the LoRA experts train.
    molf_fft: bool = True
    molf_freeze_non_linear: bool = False
    ...
    def __post_init__(self):
        ...
        if self.mode == 'lora':
            ...
            self.molf_config = MoLFConfig(
                ...
                molf_fft=self.molf_fft,
                freeze_non_linear=self.molf_freeze_non_linear,
            )
            # 'molf' keeps the FFT expert; 'mol<ranks>' is MoLF-Efficient (base
            # weight frozen, LoRA experts only). 'fnl' marks frozen non-linear params.
            if self.molf_fft:
                molf_mode_tag = "molf"
            else:
                molf_mode_tag = "mol" + "".join(f"r{r}" for r in self.molf_lora_ranks)
            if self.molf_freeze_non_linear:
                molf_mode_tag = molf_mode_tag + "fnl"
            self.run_name = f"{molf_mode_tag}_{model_name}_{self.dataset_type}_k={self.molf_topk}_scf={self.molf_score_fn}"
        else:
